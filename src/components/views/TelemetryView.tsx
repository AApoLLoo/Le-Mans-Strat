    ...
    <div className="h-full w-full bg-[#050a10] p-3 lg:p-4 flex flex-col gap-3 overflow-hidden font-display">
          ...
          <div className="flex items-center gap-6">
            <div className="text-right">
                <div className="text-[9px] text-slate-500 font-bold uppercase tracking-widest flex items-center justify-end gap-1">
                    <Clock size={10}/> Last Lap
                </div>
                <div className="flex items-baseline gap-2">
                    <span className="font-mono text-xl font-bold text-white">{formatLapTime(curLap)}</span>
                </div>
            </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-3 overflow-hidden">
        ...
        <div className="flex-[2] bg-slate-900/40 border border-white/5 rounded-xl p-4 flex flex-col relative overflow-hidden backdrop-blur-sm">
            <div className="absolute top-0 right-0 p-3 opacity-5 pointer-events-none"><Disc size={120} /></div>
        ...
        <div className="flex-[3] flex flex-col gap-3 min-h-0">
            ...
            <div className="flex-1 flex gap-3 min-h-0">
                ...
                <div className="flex-[3] bg-slate-900/40 border border-white/5 rounded-xl p-4 flex flex-col justify-center relative group">
                    ...
                    {isHypercar && (
                        <div className="mt-3 flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-mono text-green-400 font-bold">{Math.round(batterySoc)}%</span>
                        </div>
                    )}
                </div>
                ...
                <div className="flex-[2] bg-slate-900/40 border border-white/5 rounded-xl p-4 flex flex-col justify-center items-center relative overflow-hidden">
                    ...
                    <span className="text-6xl font-black text-white italic tracking-tighter font-mono drop-shadow-lg">
                ...
                <div className="mt-auto flex flex-col gap-2 z-10">
                    <div className="flex gap-2 h-2">
                        <div className="flex-1 bg-slate-800 rounded-full overflow-hidden flex flex-col-reverse relative">
                            <div className="absolute inset-0 bg-red-600 transition-all duration-75 origin-left" style={{ width: `${brake * 100}%` }}></div>
                        </div>
                        <div className="flex-1 bg-slate-800 rounded-full overflow-hidden flex flex-col-reverse relative">
                            <div className="absolute inset-0 bg-emerald-500 transition-all duration-75 origin-left" style={{ width: `${throttle * 100}%` }}></div>
                        </div>
                    </div>
                    ...
                    <div className="w-full bg-slate-950 h-6 lg:h-8 rounded-md overflow-hidden border border-slate-700 relative">
                        ...
                        <div className={`h-full transition-all duration-75 ease-linear ${rpm > maxRpm * 0.95 ? 'bg-red-500 shadow-[0_0_20px_rgba(239,68,68,0.8)]' : 'bg-gradient-to-r from-indigo-600 to-cyan-400'}`} style={{ width: `${(rpm / maxRpm) * 100}%` }}></div>
                        ...
                        <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs font-bold text-white drop-shadow-md font-mono">{Math.round(rpm)}</span>
                    </div>
                </div>
            </div>

            {/* LIGNE 3 : LEADER BENCHMARK */}
            <div className="flex gap-3 min-h-0 shrink-0">
                <div className="flex-1 bg-slate-900/40 border border-white/5 rounded-xl p-3 flex items-center justify-between overflow-hidden relative">
                    <div className="flex items-center gap-3 z-10">
                        <div className="p-2 bg-indigo-500/10 rounded-lg text-indigo-400"><Flag size={18}/></div>
                        <div>
                            <div className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">My Laps</div>
                            <div className="text-2xl font-black text-white italic tracking-tighter">{laps}</div>
                        </div>
                    </div>
                    <div className="absolute right-[-5px] bottom-[-5px] opacity-10 rotate-12"><Trophy size={40}/></div>
                </div>

                <div className="flex-1 bg-slate-900/40 border border-yellow-500/20 rounded-xl p-3 flex items-center justify-between overflow-hidden relative">
                    <div className="flex items-center gap-3 z-10 w-full">
                        <div className="p-2 bg-yellow-500/10 rounded-lg text-yellow-400"><Crown size={18}/></div>
                        <div className="flex-1 min-w-0">
                            <div className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Leader Bench.</div>
                            <div className="flex justify-between items-end">
                                <div className="text-xl font-black text-white italic tracking-tighter">{leaderLaps || 0} <span className="text-xs text-slate-500 not-italic font-normal">Laps</span></div>
                                <div className="text-xs font-mono text-yellow-400 font-bold bg-yellow-950/30 px-1.5 py-0.5 rounded border border-yellow-500/20">Avg: {formatLapTime(leaderAvgLapTime || 0)}</div>
                            </div>
                        </div>
                    </div>
                </div>
