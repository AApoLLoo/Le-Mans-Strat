...
const TelemetryView: React.FC<TelemetryViewProps> = ({ telemetryData, isHypercar, isLMGT3, position, avgLapTimeSeconds, weather, airTemp, trackWetness }) => { 
  ...
  // Sécurisation des valeurs numériques pour les tours
  const moyLap = Number(telemetryData.curLap) || 0; // Utilisation curLap ou autre champ moyen si dispo
  ...
  // On force la conversion en Nombre et on gère les valeurs nulles/undefined
  ...
  // Sécurisation des moyennes pour affichage
                    ...
                    <div className="flex-1 w-full bg-slate-950 rounded-lg overflow-hidden border border-white/10 relative shadow-inner min-h-[40px]">
                        ...
                        <div className="absolute inset-0 flex items-center justify-between px-4">
                            ...
                            <div className="text-[10px] text-right text-white/80 font-mono leading-tight">
                                <div>LAST: <span className="font-bold">{isVE ? lastLapCons.toFixed(1) + '%' : lastLapCons.toFixed(2)}</span></div>
                                <div>AVG: <span className="font-bold">{isVE ? avgCons.toFixed(1) + '%' : avgCons.toFixed(2)}</span></div>
                            </div>
                        </div>
                    </div>
