...
// Adresse de ton VPS API OVH
    ...
    export const useRaceData = (teamId: string) => {
        ...
        const [gameState, setGameState] = useState<GameState>({
            ...
            isRaceRunning: false,
            trackName: "WAITING...",
            trackLength: 0, // Ajouté pour le fix map
            sessionType: "-",
            weather: "SUNNY",
            scActive: false,
            yellowFlag: false,
        ...
        // --- CALCULATEUR STRATÉGIQUE (V3 - CORRECTION VE/FUEL RATIO) ---
        const strategyData: StrategyData = useMemo(() => {
            ...
            while (simulationLap < totalLapsTarget) {
                ...
                // 3. RECUPERATION DU RATIO (Défaut 1.0)
                // Si l'utilisateur n'a rien mis, on considère 100% (1.0)
                const ratio = config.fuelEnergyRatio !== undefined ? config.fuelEnergyRatio : 1.0;

                // 4. CALCULS CONSOMMATION
                let veNeeded = 0;
                let veDisplay = "-";
                let fuelNeeded = 0;
                let fuelDisplay = "";
                ...
                if (useVE) {
                    ...
                    // FORMULE MAGIQUE : Fuel = (VE% * Ratio) * (Capacité / 100)
                    // Ex: 40% VE * Ratio 0.9 = 36% du réservoir
                ...
                stints.push({
                    ...
                    fuelEnergyRatio: ratio, // <--- On passe la valeur à la vue
                    driver: d,
                    driverId: d.id,
